...
const straightSumOrProduct = (
  ...
  return straightSumOrProduct(sum, currentSum + first, rest) ||
    straightSumOrProduct(sum, currentSum * first, rest);
};
...
const straightSumOrProductOrOR = (
  sum: number,
  currentSum: number,
  numArray: number[],
): boolean => {
  ...
  const [first, ...rest] = numArray;
  return straightSumOrProductOrOR(sum, Number(`${currentSum}${first}`), rest) ||
    straightSumOrProductOrOR(sum, currentSum + first, rest) ||
    straightSumOrProductOrOR(sum, currentSum * first, rest);
};
